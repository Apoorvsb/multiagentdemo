...
class TestMockProductApiCall:
    ...
    def test_returns_products_matching_category(self):
        row = {
            "product_id": "P001", "name": "HP Laptop 15", "brand": "HP",
            "category": "laptops", "price": 45000.0, "rating": 4.2,
            "availability": True, "description": "Good laptop",
        }
    ...
    def test_with_brand_filter(self):
        row = {
            "product_id": "P002", "name": "Samsung Galaxy Watch",
            "brand": "Samsung", "category": "smartwatch", "price": 12000.0,
            "rating": 4.5, "availability": True, "description": "Smart watch",
        }
...
class TestExtractPreferences:
    ...
    def test_basic_extraction(self):
        ...
        llm_json = '{"keywords": ["laptop"], "max_price": 50000, "min_price": null, ' \
    ...
    def test_regex_fallback_extracts_price_when_llm_misses(self):
        ...
        llm_json = '{"keywords": ["headphones"], "max_price": null, "min_price": null, ' \
        ...
        assert prefs.get("max_price") == 1500.0, \
            f"Expected regex to catch 1500 but got {prefs.get('max_price')}"
    ...
    def test_brand_carryforward_from_history(self):
        ...
        llm_json = '{"keywords": null, "max_price": 70000, "min_price": null, ' \
                   '"category": null, "brand": null, "min_rating": null, "max_rating": null}'
        with patch("agents.product_agent.llm") as mock_llm:
            mock_llm.invoke.return_value = self._mock_llm(llm_json)
            result = self._run(state)
        ...
        prefs = result["search_preferences"]
        assert prefs.get("brand") is not None and "HP" in prefs["brand"], \
    ...
    def test_rating_regex_fallback(self):
        ...
        llm_json = '{"keywords": ["phone"], "max_price": null, "min_price": null, ' \
                   '"category": null, "brand": null, "min_rating": null, "max_rating": null}'
        with patch("agents.product_agent.llm") as mock_llm:
            mock_llm.invoke.return_value = self._mock_llm(llm_json)
            result = self._run(state)
        ...
        prefs = result["search_preferences"]
        assert prefs.get("min_rating") == 4.0, \
            f"Expected 4.0 min_rating from regex but got: {prefs.get('min_rating')}"

    def test_min_price_regex_fallback(self):
        state = make_state(current_input="show me laptops above 40000 rupees")
        llm_json = '{"keywords": ["laptop"], "max_price": null, "min_price": null, ' \
                   '"category": null, "brand": null, "min_rating": null, "max_rating": null}'
        with patch("agents.product_agent.llm") as mock_llm:
            mock_llm.invoke.return_value = self._mock_llm(llm_json)
            result = self._run(state)
        ...
        prefs = result["search_preferences"]
        assert prefs.get("min_price") == 40000.0, \
            f"Expected 40000 min_price but got: {prefs.get('min_price')}"
...
class TestMlflowHelperSpans:
    def test_log_llm_span_returns_cost(self):
        ...
        cost = log_llm_span(
            span_name="test_span", prompt_text="hello", response_text="world",
            input_tokens=100, output_tokens=50, model="llama-3.1-8b-instant",
            prompt_name="test_prompt", prompt_version=1,
        )
        assert isinstance(cost, float)
        assert cost >= 0
    ...
    def test_log_tool_span_does_not_raise(self):
        ...
        log_tool_span(
            span_name="test_tool", tool_name="mock_api",
            tool_input={"key": "value"}, tool_output={"result": "ok"},
        )
...
class TestSearchProducts:
    ...
    def test_search_products_stores_results(self):
        ...
        rows  = [{"product_id": "P1", "name": "HP Laptop", "brand": "HP",
                  "category": "laptops", "price": 45000.0, "rating": 4.2,
                  "availability": True, "description": "Good"}]
...
class TestRankAndFilter:
    def test_rank_and_filter_with_llm(self):
        ...
        products = [
            {"name": "HP Laptop", "price": 45000, "rating": 4.2,
             "description": "Good laptop", "brand": "HP"},
            {"name": "Dell Laptop", "price": 50000, "rating": 4.5,
             "description": "Better laptop", "brand": "Dell"},
        ]
        state = make_state(
        ...
        with patch("agents.product_agent.llm") as mock_llm, \
             patch("agents.product_agent.mlflow") as mock_mf:
...
class TestMockProductApiCallKeywords:
    def test_phone_keyword_uses_phone_patterns(self):
        from agents.product_agent import mock_product_api_call
        rows = [{"product_id": "P5", "name": "Samsung Galaxy S24", "brand": "Samsung",
                 "category": "smartphones", "price": 75000.0, "rating": 4.6,
                 "availability": True, "description": "Flagship phone"}]
        with patch("agents.product_agent.get_conn") as mock_gc:
            mock_db(mock_gc, fetchall=rows)
