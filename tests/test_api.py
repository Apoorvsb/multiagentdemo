...
def client():
    ...
    with patch("main.setup_mlflow"), patch("main.create_tables"), patch("database.get_conn") as mock_gc:
...
class TestLogin:
    def test_valid_user_login(self, client):
        with patch("main.user_exists", return_value=True), patch(
            "main.get_or_create_session", return_value="sess-abc"
...
class TestChat:
    ...
    def test_valid_chat_returns_response(self, client):
        ...
        with patch("main.get_session_row", return_value=session_row), patch(
            "main.get_or_create_session", return_value="sess-ok"
        ), patch("main.load_conversation_history", return_value=[]), patch("main.save_message"), patch(
            "main.update_session_agent"
        ), patch(
            "main.pipeline"
        ) as mock_pipe, patch(
            "main.mlflow"
    ...
    def test_expired_session_returns_400(self, client):
        ...
        with patch("main.get_session_row", return_value=session_row), patch(
            "main.get_or_create_session", side_effect=ValueError("Session expired")
        ):
            resp = client.post(
                "/chat",
                json={"message": "hello"},
