...
async def register(body: RegisterRequest):
    ...
    return {
        ...
        "name": body.name,
        "message": (
            "Registration successful." if not is_guest else "Guest account created. You can browse products only."
        ),
        "existing_user": False,
        "is_guest": is_guest,
    }
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                for t in templates:
                    ...
                    events = [
                        {
                            "time": str(today - timedelta(days=t["days_ago"])) + "T09:00:00Z",
                            "status": "Order placed",
                        },
                        {
                    ...
                    if t["status"] == "DELIVERED":
                        events.append(
                            {
                                "time": str(today + timedelta(days=t["eta_offset"])) + "T15:00:00Z",
                                "status": "Delivered",
                            }
                        )
                    if t["status"] == "DELAYED":
